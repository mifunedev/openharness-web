...
>
> **Spellings, as of 2026-09-02.** The lifecycle verbs below have been rewritten to today's form: `oh sandbox install docker --name <name>` creates a sandbox from any directory, `oh shell <name>` opens one, and `oh destroy <name>` removes it. The prose about a container with every agent CLI "preinstalled" is archived history and left as written — nothing installs at boot today, and each agent enters the running sandbox through `oh harness install <id>`.
...
```bash
oh sandbox install docker --name my-agent   # create + start the sandbox
oh shell my-agent                           # drop into a zsh inside it
oh destroy my-agent                         # blow it away
```
...
Or, after the first sandbox: `oh sandbox install docker --name <name>` from anywhere.

Full docs at [github.com/mifunedev/openharness](https://github.com/mifunedev/openharness/tree/main/docs). Source at [github.com/mifunedev/openharness](https://github.com/mifunedev/openharness). Issues and PRs welcome — the fastest way to make BYOH better is to bring _your_ harness rules and tell me what's missing.
