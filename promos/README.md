...
Use banner recipes when a promo post needs a visual. The recipe is the source of truth; rendered images are outputs.

1. Create or edit a JSON recipe under `promos/banner-recipes/`.
2. Keep visual decisions in explicit slots: headline lines, proof path, CTA, source screenshot, crop/slot, output paths, and review checklist.
3. Render the card:
...
This is intentionally Canva-like but source-controlled: no AI image generation, no hidden edits, no one-off screenshots pasted into a design tool.
